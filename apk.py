...
max_pairs_to_scan = st.sidebar.slider(
    "Liczba par w skanerze", 5, 30, 10, step=5
)

st.sidebar.markdown("---")
st.sidebar.subheader("💰 Zarządzanie Kapitałem")
amount_usdt = st.sidebar.number_input(
    "Wielkość pozycji bazowej (USDT)", value=50.0, step=10.0
)
leverage = st.sidebar.slider(
    "Dźwignia (Leverage)", 1, 20, 5
)
...
if st.sidebar.button("🚨 KILL SWITCH (Zamknij wszystko)", type="primary"):
  ...
  if api_key and api_secret and api_password:
    ...
    st.sidebar.warning(
        "Uzupełnij klucze API, aby wykonać awaryjne zamknięcie."
    )
...
st.markdown(
    "Profesjonalna platforma handlowa z wieloparowym skanerem płynności, filtrem"
    " trendu EMA50 oraz pełną ochroną kapitału."
)
...
with c_start:
  if st.button(
      "▶ Uruchom Autonomiczny Handel", type="primary", use_container_width=True
  ):
...
if api_key and api_secret and api_password:
  try:
    ...
    with tab1:
      ...
      if st.button(
          "🔄 Wykonaj Cykl Skanowania i Analizy Rynku",
          type="primary",
          use_container_width=True,
      ):
        with st.spinner(
            "Skanowanie giełdy Bitget, analiza płynności oraz wskaźników..."
        ):
          # 1. Skanowanie topowych par
          active_symbols = get_top_volume_pairs(
              exchange, limit=max_pairs_to_scan, m_type=market_type
          )
          st.info(
              f"Pomyślnie pobrano top {len(active_symbols)} płynnych par o"
              " największym obrocie."
          )

          # 2. Pobranie aktywnych pozycji
          ...
          for symbol in active_symbols:
            ...
            if symbol in active_positions_map:
              ...
              pos_side = pos["side"] # 'long' lub 'short'
              entry_price = float(pos["entryPrice"])
              contracts = float(pos["contracts"])
              pnl_pct = float(pos.get("percentage", 0))

              st.write(
                  f"🔍 **{symbol}** | Aktywna: **{pos_side.upper()}** | Wejście:"
                  f" `{entry_price}` | PnL: `{pnl_pct:.2f}%`"
              )
              ...
              # Wyjście po odwróceniu trendu (po minięciu czasu Cooldown)
            ...
            # --- B. SZUKANIE NOWYCH OKAZJI Z FILTREM TRENDU EMA50 ---
              ...
              # Wejście w LONG: Cena nad EMA50 + potwierdzenie MACD
              if (
              ...
              # Wejście w SHORT: Cena pod EMA50 + potwierdzenie MACD
              elif (
                  current_price < last_ema50
                  and prev_macd >= prev_signal
                  and last_macd < last_signal
    ...
    with tab2:
      ...
      if st.session_state.trade_history:
        ...
        st.info(
            "Brak zarejestrowanych zdarzeń w bieżącej sesji. Uruchom cykl"
            " skanowania."
        )
    ...
    st.warning(
        "👈 Uzupełnij dane dostępowe API w panelu bocznym, aby uruchomić"
        " interfejs terminala."
    )
