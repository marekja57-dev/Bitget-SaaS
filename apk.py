...
# =====================================================================
st.subheader("🤖 Bot Trendu EMA (Dynamiczny Margines pod Ryzyko + Twarde Ostrza Guard Clause)")
...
# =====================================================================
# PANCERNA LOGIKA BOTA Z TWARDYMI BLOKADAMI ANTIMELIOWYMI
# =====================================================================
...
if futures_ex and db_active_check and max_active_pairs > 0:
    try:
        ...
        # 1. Zarządzanie otwartymi pozycjami (Wyjścia)
        for sym, pos in list(exchange_positions.items()):
            ...
            try:
                ...
                if not closed_position:
                    if pos_side == 'long' and last_fast < last_slow:
                        ...
                        st.session_state.trade_history.insert(0, {
                            ...
                            "Para": sym,
                            "Cena": f"{current_price:.4f}",
                            "Info": "Fast EMA < Slow EMA (Zamknięta świeca)"
                        })
                        closed_position = True
                ...
                if closed_position:
                    # 10 MINUT TWARDEGO COOLDOWNU DLA PARY (ZABRANIA NATYCHMIASTOWEGO PONOWNEGO WEJŚCIA)
                    st.session_state.symbol_cooldowns[sym] = time.time() + 600
        ...
        # 2. Skanowanie i otwieranie nowych pozycji (Wejścia)
        if get_db_bot_status(st.session_state.user_id):
            ...
            if total_active_and_locked < max_active_pairs and fut_free >= 5.0:
                for sym in top_symbols:
                    ...
                    # GUARD 1: Cooldown czasowy (10 minut przerwy od ostatniej akcji na parze)
                    cooldown_until = st.session_state.symbol_cooldowns.get(sym, 0)
                    if time.time() < cooldown_until:
                        continue

                    # GUARD 2: Czy para już ma pozycję lub blokadę sesyjną
                    if sym in exchange_positions or sym in st.session_state.locked_symbols:
                        continue

                    # GUARD 3: Sprawdzenie otwartych zleceń oczekujących na giełdzie dla tej pary
                    try:
                        open_ord = futures_ex.fetch_open_orders(sym)
                        if open_ord and len(open_ord) > 0:
                            continue
                    except Exception:
                        pass
                    ...
                    try:
                        ...
                        if is_bullish or is_bearish:
                            ...
                            if contracts_prec > 0:
                                ...
                                # USTAWIENIE TWARDEGO COOLDOWNU 10 MINUT
                                st.session_state.symbol_cooldowns[sym] = time.time() + 600
                                time.sleep(0.3)
                    except Exception:
                        pass
