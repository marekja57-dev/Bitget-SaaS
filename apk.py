...
# =====================================================================
# GŁÓWNA LOGIKA HANDLOWA (WYWOLYWANA W TLE)
# =====================================================================
def run_trading_logic_cycle():
    # 1. Sniper Nowych Listingów
    ...
    # 2. Awaryjna kontrola SL / TP oraz zamykanie pozycji Spot
    if spot_ex and st.session_state.active_spot_trades:
        try:
            s_tickers = spot_ex.fetch_tickers(list(st.session_state.active_spot_trades.keys()))
            for sym, tinfo in list(st.session_state.active_spot_trades.items()):
    ...
    # 3. Awaryjna kontrola SL / TP Futures
    ...
    # 5. Automatyczne zamykanie pozycji Futures przy odwróceniu trendu (MACD) oraz Bot Futures
    if futures_ex:
        try:
            current_positions = futures_ex.fetch_positions()
            for pos in current_positions:
...
# Uruchomienie bezpiecznego wątku w tle serwera (Worker)
@st.cache_resource
def start_autonomous_background_worker():
    def worker_loop():
        while True:
            try:
                run_trading_logic_cycle()
            except Exception:
                pass
            time.sleep(3)
...
start_autonomous_background_worker()
...
# Odświeżanie interfejsu (UI refresh co określony interwał)
